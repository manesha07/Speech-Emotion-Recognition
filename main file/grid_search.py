...
for model, params in classification_grid_parameters.items():
    ...
    # if model.__class__.__name__ == "SVC":
        # in case of a K-Nearest neighbors algorithm
        # set number of neighbors to the length of emotions
        # params = {'C'=0.001, 'gamma'=0.001, 'kernel'='poly'}
    # my_model = SVC(C=0.001, gamma=0.001, kernel='poly')

    # d = EmotionRecognizer(model='SVC', emotions=['sad', 'neutral', 'happy'], balance=True, verbose=0)
    ...
    # x_train,y_train,x_test,y_test = d.grid_search1()
    # my_model.fit(x_train,y_train)
    # c = my_model.predict(x_test)
    #
    # print(accuracy_score(x_test,c))


print(f"[+] Pickling best classifiers for {emotions}...")
pickle.dump(best_estimators, open(f"grid/best_classifiers.pickle", "wb"))

# pickle.dump(my_model, open(f"grid/best_classifiers.pickle", "wb"))
